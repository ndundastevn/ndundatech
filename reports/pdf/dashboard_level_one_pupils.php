 ...
 */
                 ...
                 <body style="margin:1px;">
                     ...
                     <div class="list_header" align="center">
                         <h3>
                             ...
                             ECDE Enrollments Breakdown Per Subcounties<br>
                         </h3>                        
                     </div>
                             ...
                             ';
                                 ...
                                 if (mysqli_num_rows($pdf_sql) > 0) {
                                     while ($row = mysqli_fetch_array($pdf_sql)) {
                                         ...
                                         /* Get Number of Playgroup pupils in each subcounty */
                                         $query = "SELECT COUNT(*) FROM pupils p 
                                         INNER JOIN ecde_centers c ON c.center_id = p.pupil_center_id 
                                         INNER JOIN ward w ON w.ward_id = c.center_ward_id 
                                         WHERE w.ward_sub_couty_id   = '{$row['sub_county_id']}' AND p.pupil_academic_level = 'Playgroup'";
                                         $stmt = $mysqli->prepare($query);
                                         $stmt->execute();
                                         ...
                                         /* Get Number of PP1 in each subcounty */
                                         $query = "SELECT COUNT(*) FROM pupils p 
                                         INNER JOIN ecde_centers c ON c.center_id = p.pupil_center_id 
                                         INNER JOIN ward w ON w.ward_id = c.center_ward_id 
                                         WHERE w.ward_sub_couty_id   = '{$row['sub_county_id']}' AND p.pupil_academic_level = 'PP1'";
                                         $stmt = $mysqli->prepare($query);
                                         $stmt->execute();
                                         ...
                                         /* Get Number of PP2 in each subcounty */
                                         $query = "SELECT COUNT(*) FROM pupils p 
                                         INNER JOIN ecde_centers c ON c.center_id = p.pupil_center_id 
                                         INNER JOIN ward w ON w.ward_id = c.center_ward_id 
                                         WHERE w.ward_sub_couty_id   = '{$row['sub_county_id']}' AND p.pupil_academic_level = 'PP2'";
                                         $stmt = $mysqli->prepare($query);
                                         $stmt->execute();
                                         ...
                                         $stmt = $mysqli->prepare($query);
                                         $stmt->execute();
                                         $stmt->bind_result($total_trainees_count);
                                         $stmt->fetch();
                                         $stmt->close();
 
                                         // Accumulate totals
                                         $total_centers += $number_of_centers;
                                         $total_playgroup += $baby_class;
                                         $total_pp1 += $pp1;
                                         $total_pp2 += $pp2;
                                         ...
                                         $html .=
                                             ...
                                             <tr>
                                                 ...
                                                 <td>' . number_format($total_trainees_count) . '</td>
                                             </tr>
                                         ';
                                         $cnt++;
                                     }
 ...
 $dompdf->render();
 $dompdf->stream('ECDE Enrollments Breakdown Per Subcounties', array("Attachment" => 1));
