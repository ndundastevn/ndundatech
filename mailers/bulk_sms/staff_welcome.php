...
function sendBulkSMS($sender_id, $api_key, $client_id, $numbers, $message, $isUnicode = true, $isFlash = false)
{
    ...
    // Check for cURL errors -> Let this sucker run for tests only
   /*  if (curl_errno($curl)) {
...
if (mysqli_num_rows($sms_configs) > 0) {
    while ($configs = mysqli_fetch_array($sms_configs)) {
        ...
        $numbers = [$user_phone];
        $message = 
        "Greetings $user_names!  Welcome to Government of Makueni County - ECDE MIS. Your account is active and for first login, use the folowing credentials:- Login url: https://ecde.makueni.go.ke,  Login email: $user_email, Default password: Makueni102";
